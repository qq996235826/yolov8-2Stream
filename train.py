...
if __name__ == '__main__':
    ...
    # val
    model = YOLO('weight/best.pt')  # load an official model
    metrics = model.val(data="dataset/yolo-10000.yaml", split=['test_rgb', 'test_depth'])

    # 推理
    # model = YOLO('weight/best.pt')  # load a custom model
    #
    # # Predict with the model
    # results = model(['color.png', 'depth_viridis.png'])  # predict on an image*
    # # 保存结果图
