...
class BaseModel(nn.Module):
    ...
    def forward(self, x, *args, **kwargs):
        ...
        # 改动，由于传递参数的方式过多，且无法统一，所以这里进行判断，如果depth被传到args里的话，就把他拿出来
    ...
    def _predict_once(self, x, x2, profile=False, visualize=False):
        ...
        y, dt = [], []  # outputs
        for m in self.model:
            if m.f != -1 and m.f != -2 and m.f != -4:  # if not from previous layer
                # x = y[m.f] if isinstance(m.f, int) else [x if j == -1 else y[j] for j in m.f]  # from earlier layers
    ...
    def loss(self, batch, preds=None):
        ...
        preds = self.forward([batch['img_rgb'], batch['img_depth']]) if preds is None else preds
