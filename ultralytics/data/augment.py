...
class Mosaic(BaseMixTransform):
    ...
    def _mosaic4(self, labels):
        ...
        for i in range(4):
            ...
            img_depth = labels_patch['img_depth']
            h, w = labels_patch.pop('resized_shape')

            # Place img in img4
            if i == 0:  # top left
                img4_rgb = np.full((s * 2, s * 2, img_rgb.shape[2]), 114, dtype=np.uint8)  # base image with 4 tiles
                img4_depth = np.full((s * 2, s * 2, img_depth.shape[2]), 114, dtype=np.uint8)  # base image with 4 tiles
            ...
            img4_depth[y1a:y2a, x1a:x2a] = img_depth[y1b:y2b, x1b:x2b]  # img4[ymin:ymax, xmin:xmax]
            padw = x1a - x1b
            padh = y1a - y1b
        ...
        final_labels['img_depth'] = img4_depth
        return final_labels
...
class RandomPerspective:
    ...
    def affine_transform(self, img_rgb, img_depth, border):
        ...
        if (border[0] != 0) or (border[1] != 0) or (M != np.eye(3)).any():  # image changed
            if self.perspective:
                ...
                img_depth = cv2.warpPerspective(img_depth, M, dsize=self.size, borderValue=(114, 114, 114))
            else:  # affine
                img_rgb = cv2.warpAffine(img_rgb, M[:2], dsize=self.size, borderValue=(114, 114, 114))
                img_depth = cv2.warpAffine(img_depth, M[:2], dsize=self.size, borderValue=(114, 114, 114))
        return img_rgb, img_depth, M, s
    ...
    def __call__(self, labels):
        ...
        img_rgb, img_depth, M, scale = self.affine_transform(img_rgb, img_depth, border)
...
class RandomHSV:
    ...
    def __call__(self, labels):
        ...
        if self.hgain or self.sgain or self.vgain:
            ...
            hue_depth, sat_depth, val_depth = cv2.split(cv2.cvtColor(img_depth, cv2.COLOR_BGR2HSV))
            ...
            im_hsv_depth = cv2.merge(
                (cv2.LUT(hue_depth, lut_hue), cv2.LUT(sat_depth, lut_sat), cv2.LUT(val_depth, lut_val)))
            cv2.cvtColor(im_hsv, cv2.COLOR_HSV2BGR, dst=img_rgb)  # no return needed
            cv2.cvtColor(im_hsv_depth, cv2.COLOR_HSV2BGR, dst=img_depth)  # no return needed
        return labels
...
class RandomFlip:
    ...
    def __call__(self, labels):
        ...
        labels['img_depth'] = np.ascontiguousarray(img_depth)
...
class LetterBox:
    ...
    def __call__(self, labels=None, image=None):
        ...
        if isinstance(image, np.ndarray) and image.ndim == 3:
            img_rgb = labels.get('img_rgb') if image is None else image
            img_depth = labels.get('img_depth') if image is None else image
        else:
            img_rgb = labels.get('img_rgb') if image is None else image[0]
            img_depth = labels.get('img_depth') if image is None else image[1]
        ...
        if shape[::-1] != new_unpad:  # resize
            ...
            img_depth = cv2.resize(img_depth, new_unpad, interpolation=cv2.INTER_LINEAR)
        top, bottom = int(round(dh - 0.1)) if self.center else 0, int(round(dh + 0.1))
        left, right = int(round(dw - 0.1)) if self.center else 0, int(round(dw + 0.1))
        img_rgb = cv2.copyMakeBorder(img_rgb, top, bottom, left, right, cv2.BORDER_CONSTANT,
                                     value=(114, 114, 114))  # add border
        img_depth = cv2.copyMakeBorder(img_depth, top, bottom, left, right, cv2.BORDER_CONSTANT,
                                       value=(114, 114, 114))  # add border
...
class CopyPaste:
    ...
    def __call__(self, labels):
        ...
        if self.p and len(instances.segments):
            ...
            im_new_depth = np.zeros(im_depth.shape, np.uint8)
            ...
            for j in random.sample(list(indexes), k=round(self.p * n)):
                ...
                cv2.drawContours(im_new_depth, instances.segments[[j]].astype(np.int32), -1, (1, 1, 1), cv2.FILLED)

            result = cv2.flip(im_rgb, 1)  # augment segments (flip left-right)
            result_depth = cv2.flip(im_depth, 1)  # augment segments (flip left-right)
            i = cv2.flip(im_new, 1).astype(bool)
            i_depth = cv2.flip(im_new_depth, 1).astype(bool)
            im_rgb[i] = result[i]
            im_depth[i_depth] = result_depth[i_depth]

        labels['img_rgb'] = im_rgb
        labels['img_depth'] = im_depth
        labels['cls'] = cls
        labels['instances'] = instances
        return labels
...
class Albumentations:
    ...
    def __call__(self, labels):
        ...
        if len(cls):
            ...
            labels['instances'].normalize(*im_rgb.shape[:2][::-1])
            bboxes = labels['instances'].bboxes
            # TODO: add supports of segments and keypoints
            if self.transform and random.random() < self.p:
                new_rgb = self.transform(image=im_rgb, bboxes=bboxes, class_labels=cls)  # transformed
                new_depth = self.transform(image=im_depth, bboxes=bboxes, class_labels=cls)  # transformed
                if len(new_rgb['class_labels']) > 0:  # skip update if no bbox in new im
                    labels['img_rgb'] = new_rgb['image']
                    labels['img_depth'] = new_depth['image']
                    labels['cls'] = np.array(new_rgb['class_labels'])
                    bboxes = np.array(new_rgb['bboxes'], dtype=np.float32)
...
class Format:
    ...
    def __call__(self, labels):
        ...
        img_depth = labels.pop('img_depth')
        h, w = img_rgb.shape[:2]
        cls = labels.pop('cls')
        instances = labels.pop('instances')
        ...
        if self.return_mask:
            if nl:
                ...
                masks = torch.zeros(1 if self.mask_overlap else nl, img_rgb.shape[0] // self.mask_ratio,
                                    img_rgb.shape[1] // self.mask_ratio)
            labels['masks'] = masks
        if self.normalize:
            instances.normalize(w, h)
        labels['img_rgb'] = self._format_img(img_rgb)
        labels['img_depth'] = self._format_img(img_depth)
